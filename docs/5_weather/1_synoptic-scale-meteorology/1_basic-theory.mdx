...
### 渦度保存
:::tip[絶対渦度保存]
$\displaystyle\frac{d(\zeta + f)}{dt}=\frac{d\zeta}{dt} + \beta v = -(\zeta + f)\nabla\cdot\bm{v} - \left(\frac{\partial\omega}{\partial x}\frac{\partial v}{\partial p} - \frac{\partial\omega}{\partial y}\frac{\partial u}{\partial p}\right) + \nabla\times\bm{F_r}~~\left(\beta = \frac{df}{dy}\right)$
...
:::tip[安定性]
**鉛直方向の安定性：静的安定度が重要**。相当温位 $\theta_e$ の鉛直分布で考えることが多い？
