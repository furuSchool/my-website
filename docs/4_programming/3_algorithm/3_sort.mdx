...
---
### ツリーソート
配列を二分探索木に変換し，最も左のノードから順に取り出す。([二分木](/docs/4_programming/3_algorithm/2_search.mdx#二分木)を参照)
- 大体 $O(n \log n)$。偏っていると $O(n^2)$
- 取り出すのは，「左の部分木を辿る $\rightarrow$ 右の部分木を辿る」ことを再帰的に行う
...
- [ヒープ](/docs/4_programming/3_algorithm/1_data-structure.mdx#ヒープ)を参照
