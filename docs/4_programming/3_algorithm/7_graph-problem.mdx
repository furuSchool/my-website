...
def KahnTopo(V, E, edges):
    ...
    sorted_g = []
    deq = list(v for v in range(E) if inedge[v]==0)
    while deq:
        v = deq.pop(0) # 入次数 0 のノードを取り出す
        for item in outedge[v]:
            ...
            if inedge[item] == 0: # 入次数が 0 になったノードを deq に追加
                ...
                if right < 0:
                    ...
                    deq.insert(left, item)
                elif item > deq[right]:
                    deq.insert(right+1,item)
                else:
                    while right - left > 1:
                        tmp = (right+left)//2
                        if item > deq[tmp]:
                            ...
                            right = tmp
                        deq.insert(right,item)
        sorted_g.append(v)
    if E != 0:
        return None # 辺が残っていれば DAG でない
    return sorted_g
```
</CodeToggle>
