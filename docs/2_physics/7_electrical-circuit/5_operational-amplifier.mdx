...
- 高周波数領域では利得が小さくなる。例えば，$A(s) = \frac{A_0}{1 + \frac{s}{\omega_p}}$ のように近似される
    ...
    - 実際にはオペアンプは 3 段以上の MOSFET で構成されており，LPF 三段重ねのような構造になっている。位相特性も，最大 $-270^\circ$ 程度の遅れがあり得り，
    これが発振の原因となっている
...
なお，帰還回路は入力を減算する (位相を反転させて足し合わせる)。この時，計算すると以下が成り立つ。
$$
A_V(s) = \frac{V_{out}}{V_{in}} = \frac{A(s)}{1 + A(s) \beta} = \frac{1}{\beta}\frac{A(s)\beta}{1 + A(s)\beta}
$$
- なお，この時の $A_V(s)$ は **閉ループ利得**，$A(s)$ は **開ループ利得**，$A(s)\beta$ は **ループ利得** と呼ばれる

この時，**発振する**，すなわち入力信号がなくとも微小な雑音によって $V_{out}$ が無限に大きくなってしまう条件は，  
**極 (この場合は $A(s)\beta + 1 = 0$ の解 ) が右半平面にあること** である。
- 実際には，電源電圧によって $V_{out}$ は制限され，$V_{out} = V_{DD}$ のようになる

これは，ナイキストの判定法を用いたり，位相が $-180^\circ$ になる周波数での $A(s)\beta$ の大きさが $1$ より小さいかどうかを調べることで確認できる。
- 位相余裕 : ゲインが $1$ になる周波数での $A(s)\beta$ の位相が $-180^\circ$ になるまでの差を位相余裕と呼ぶ
- ゲイン余裕 : 位相が $-180^\circ$ になる周波数での $A(s)\beta$ の大きさが $1$ になるまでの差をゲイン余裕と呼ぶ
...
ここから，$|A(j2\pi f_{th}) \beta| = 1$ が成り立つ $f_{th}$ を求めると，$f_{th} = \frac{f_u}{1 + \frac{R_2}{R_1}}$ となる。
よって，この周波数で位相余裕があれば発振は起きない。
- ユニティゲイン周波数 $f_u$ を用いた
- 発振を起きないようにする方法としては，遅れ位相補償 (帰還回路にコンデンサを入れ，周波数が低い段階からゲインを下げちゃう) などがある