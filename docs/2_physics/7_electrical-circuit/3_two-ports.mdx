...
また，入力同士・出力同士の関係を表す関数を **駆動点関数 (イミタンス関数)**，入力と出力の関係を表す関数を **伝達関数** と呼ぶ。
特に，$H(s) = \frac{V_2}{V_1}$ を **伝達関数** と呼ぶことが多い。
...
### F 行列・Z 行列・Y 行列・H 行列
:::tip[2 端子対回路の行列]
2 端子対回路は、以下のような行列で表現される。Z・Y 行列では，対角要素は **駆動点関数 (イミタンス関数)**，非対角要素は **伝達関数** となる。
...
- なお，駆動点関数 (イミタンス関数) の場合は，例えば $H = \frac{V_1}{I_1}$ が考えられるが，
$\frac{1}{H}$ も駆動点関数であることから零点も左半平面にあることが安定性の条件となる
